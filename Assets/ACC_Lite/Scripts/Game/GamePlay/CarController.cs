...
public class CarController : MonoBehaviour
{
    ...
    void UpdateRpmAndTorqueLogic()
    {
        ...
        if (!InCutOff)
        {
            float targetRPM = ((minRPM + 20) * AllGearsRatio[CurrentGearIndex]).Abs();
        ...
        if (AutomaticGearBox)
        {
            ...
            {
                EngineRPM = Mathf.Lerp(EngineRPM, EngineRPM * (newRatio / prevRatio), RpmEngineToRpmWheelsLerpSpeed * Time.fixedDeltaTime);
            }
